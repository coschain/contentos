...
namespace eosio {
   ...
   struct asset {
      ...
      :amount(a),symbol{s}
      {
         contento_assert( is_amount_within_range(), "magnitude of asset amount must be less than 2^62" );
         contento_assert( symbol.is_valid(),        "invalid symbol name" );
      }
      ...
      asset& operator-=( const asset& a ) {
         contento_assert( a.symbol == symbol, "attempt to subtract asset with different symbol" );
         amount -= a.amount;
         contento_assert( -max_amount <= amount, "subtraction underflow" );
         contento_assert( amount <= max_amount,  "subtraction overflow" );
         return *this;
      }

      asset& operator+=( const asset& a ) {
         contento_assert( a.symbol == symbol, "attempt to add asset with different symbol" );
         amount += a.amount;
         contento_assert( -max_amount <= amount, "addition underflow" );
         contento_assert( amount <= max_amount,  "addition overflow" );
         return *this;
      }
      ...
      asset& operator*=( int64_t a ) {
         contento_assert( a == 0 || (amount * a) / a == amount, "multiplication overflow or underflow" );
         contento_assert( -max_amount <= amount, "multiplication underflow" );
         contento_assert( amount <= max_amount,  "multiplication overflow" );
         amount *= a;
         return *this;
      }
      ...
      friend int64_t operator/( const asset& a, const asset& b ) {
         contento_assert( a.symbol == b.symbol, "comparison of assets with different symbols is not allowed" );
   ...
   struct extended_asset : public asset {
      ...
      friend extended_asset operator - ( const extended_asset& a, const extended_asset& b ) {
         contento_assert( a.contract == b.contract, "type mismatch" );
