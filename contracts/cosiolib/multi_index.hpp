...
class multi_index
{
   private:
      ...
      struct index {
         public:
            ...
            struct const_iterator : public std::iterator<std::bidirectional_iterator_tag, const T> {
               public:
                  ...
                  const_iterator& operator++() {
                     ...
                     contento_assert( _item != nullptr, "cannot increment end iterator" );
                  ...
                  const_iterator& operator--() {
                     ...
                     if( !_item ) {
                        ...
                        contento_assert( prev_itr >= 0, "cannot decrement iterator at beginning of index" );
                     }
            ...
            const T& get( const secondary_key_type& secondary, const char* error_msg = "unable to find secondary key" )const {
               ...
               contento_assert( result != cend(), error_msg );
               return *result;
            }
            ...
            const_iterator iterator_to( const T& obj ) {
               ...
               contento_assert( objitem.__idx == _multidx, "object passed to iterator_to is not in multi_index" );
            ...
            void modify( const_iterator itr, account_name* payer, Lambda&& updater ) {
               contento_assert( itr != cend(), "cannot pass end iterator to modify" );

               _multidx->modify( *itr, payer, std::forward<Lambda&&>(updater) );
            }

            const_iterator erase( const_iterator itr ) {
               contento_assert( itr != cend(), "cannot pass end iterator to erase" );
            ...
            namex get_code()const  { return _multidx->get_code(); }
            namex get_scope()const { return _multidx->get_scope(); }
      ...
      const item& load_object_by_primary_iterator( int32_t itr )const {
         ...
         contento_assert( size >= 0, "error reading iterator" );
   ...
   public:
      ...
      namex get_code()const  { return _code; }
      namex get_scope()const { return _scope; }
      ...
      struct const_iterator : public std::iterator<std::bidirectional_iterator_tag, const T> {
         ...
         const_iterator& operator--() {
            ...
            if( !_item ) {
               ...
               contento_assert( prev_itr >= 0, "cannot decrement iterator at beginning of table" );
            }
      ...
      uint64_t available_primary_key()const {
         ...
         contento_assert( _next_primary_key < no_available_primary_key, "next primary key in table is at autoincrement limit");
      ...
      const_iterator iterator_to( const T& obj )const {
         const auto& objitem = static_cast<const item&>(obj);
         contento_assert( objitem.__idx == this, "object passed to iterator_to is not in multi_index" );
         return {this, &objitem};
      }
      ...
      const_iterator emplace( account_name* payer, Lambda&& constructor ) {
         ...
         contento_assert( _code == get_current_caller(), "cannot create objects in table of another contract" ); // Quick fix for mutating db using multi_index that shouldn't allow mutation. Real fix can come in RC2.
      ...
      template<typename Lambda>
      void modify( const_iterator itr, account_name* payer, Lambda&& updater ) {
         contento_assert( itr != end(), "cannot pass end iterator to modify" );
      ...
      void modify( const T& obj, account_name* payer, Lambda&& updater ) {
         ...
         contento_assert( _code == get_current_caller(), "cannot modify objects in table of another contract" ); // Quick fix for mutating db using multi_index that shouldn't allow mutation. Real fix can come in RC2.
         ...
         contento_assert( pk == obj.primary_key(), "updater cannot change primary key when modifying an object" );
      ...
      const_iterator erase( const_iterator itr ) {
         contento_assert( itr != end(), "cannot pass end iterator to erase" );
      ...
      void erase( const T& obj ) {
         ...
         contento_assert( _code == get_current_caller(), "cannot erase objects in table of another contract" ); // Quick fix for mutating db using multi_index that shouldn't allow mutation. Real fix can come in RC2.
         ...
         contento_assert( itr2 != _items_vector.rend(), "attempt to remove object that was not in multi_index" );
