...
namespace eosiosystem {
   ...
   {
      require_auth( payer );
      contento_assert( quant.amount > 0, "must purchase a positive amount" );
      ...
      contento_assert( bytes_out > 0, "must reserve a positive amount" );
   ...
   void system_contract::sellram( account_name account, int64_t bytes ) {
      ...
      contento_assert( bytes > 0, "cannot sell negative byte" );

      user_resources_table  userres( _self, account );
      auto res_itr = userres.find( account );
      contento_assert( res_itr != userres.end(), "no resource row" );
      contento_assert( res_itr->ram_bytes >= bytes, "insufficient quota" );
      ...
      contento_assert( _gstate.total_ram_stake >= 0, "error, attempt to unstake more tokens than previously staked" );
   ...
   void validate_b1_vesting( int64_t stake ) {
      ...
      contento_assert( max_claimable - claimable <= stake, "b1 can only claim their tokens over 10 years" );
   }
   ...
   {
      require_auth( from );
      contento_assert( stake_net_delta != asset(0) || stake_cpu_delta != asset(0), "should stake non-zero amount" );
      ...
      // update stake delegated from "from" to "receiver"
      {
         ...
         contento_assert( asset(0) <= itr->net_weight, "insufficient staked net bandwidth" );
         contento_assert( asset(0) <= itr->cpu_weight, "insufficient staked cpu bandwidth" );
      ...
      // update totals of "receiver"
      {
         ...
         contento_assert( asset(0) <= tot_itr->net_weight, "insufficient staked total net bandwidth" );
         contento_assert( asset(0) <= tot_itr->cpu_weight, "insufficient staked total cpu bandwidth" );
      ...
      if ( N(eosio.stake) != source_stake_from ) { //for eosio both transfer and refund make no sense
         ...
         if ( req != refunds_tbl.end() ) { //need to update refund
            ...
            contento_assert( asset(0) <= req->net_amount, "negative net refund amount" ); //should never happen
            contento_assert( asset(0) <= req->cpu_amount, "negative cpu refund amount" ); //should never happen
      ...
      // update voting power
      {
         ...
         contento_assert( 0 <= from_voter->staked, "stake for voting cannot be negative");
         if( from == N(b1) ) {
   ...
   {
      contento_assert( stake_cpu_quantity >= asset(0), "must stake a positive amount" );
      contento_assert( stake_net_quantity >= asset(0), "must stake a positive amount" );
      contento_assert( stake_net_quantity + stake_cpu_quantity > asset(0), "must stake a positive amount" );
   ...
   {
      contento_assert( asset() <= unstake_cpu_quantity, "must unstake a positive amount" );
      contento_assert( asset() <= unstake_net_quantity, "must unstake a positive amount" );
      contento_assert( asset() < unstake_cpu_quantity + unstake_net_quantity, "must unstake a positive amount" );
      contento_assert( _gstate.total_activated_stake >= min_activated_stake,
   ...
   void system_contract::refund( const account_name owner ) {
      ...
      contento_assert( req != refunds_tbl.end(), "refund request not found" );
      contento_assert( req->request_time + refund_delay <= now(), "refund is not available yet" );
