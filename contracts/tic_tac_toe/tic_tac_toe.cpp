...
struct impl {
   ...
   void on(const create& c) {
      require_auth(c.host);
      contento_assert(c.challenger != c.host, "challenger shouldn't be the same as host");

      // Check if game already exists
      games existing_host_games(code_account, c.host);
      auto itr = existing_host_games.find( c.challenger );
      contento_assert(itr == existing_host_games.end(), "game already exists");
   ...
   void on(const restart& r) {
      ...
      contento_assert(itr != existing_host_games.end(), "game doesn't exists");

      // Check if this game belongs to the action sender
      contento_assert(r.by == itr->host || r.by == itr->challenger, "this is not your game!");
   ...
   void on(const move& m) {
      ...
      contento_assert(itr->winner == N(none), "the game has ended!");
      // Check if this game belongs to the action sender
      contento_assert(m.by == itr->host || m.by == itr->challenger, "this is not your game!");
      // Check if this is the  action sender's turn
      contento_assert(m.by == itr->turn, "it's not your turn yet!");


      // Check if user makes a valid movement
      contento_assert(is_valid_movement(m.mvt, *itr), "not a valid movement!");
