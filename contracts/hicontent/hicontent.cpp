...
{
  public:
    hicontent(account_name self) : contract(self) {}
    ...
    void query()
    {
        ...
        {
            vector<namex> res = database_api().get_active_witnesses();
            for (auto x : res)
                print("witness name: ", x, "\n");
        }
...

        {
            ...
            if (st.content.size() > 0)
                print("comment body: ", st.content.begin()->second.body, "\n");
            if (st.accounts.size() > 0)
                print("accounts name: ", st.accounts.begin()->first, "\n");
