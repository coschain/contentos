...
void multisig::propose() {
   ...
   contento_assert( trx_header.expiration >= eosio::time_point_sec(now()), "transaction expired" );
   //contento_assert( trx_header.actions.size() > 0, "transaction must have at least one action" );

   proposals proptable( _self, proposer );
   contento_assert( proptable.find( proposal_name ) == proptable.end(), "proposal with the same name exists" );
   ...
   contento_assert( res > 0, "transaction authorization failed" );
...
void multisig::approve( account_name proposer, name proposal_name, permission_level level ) {
   ...
   contento_assert( itr != apps.requested_approvals.end(), "approval is not on the list of requested approvals" );
...
void multisig::unapprove( account_name proposer, name proposal_name, permission_level level ) {
   ...
   contento_assert( itr != apps.provided_approvals.end(), "no approval previously granted" );
...
void multisig::cancel( account_name proposer, name proposal_name, account_name canceler ) {
   ...
   if( canceler != proposer ) {
      contento_assert( unpack<transaction_header>( prop.packed_transaction ).expiration < eosio::time_point_sec(now()), "cannot cancel until expiration" );
   }
