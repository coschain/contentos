...
void apply_social_post() {
   ...
   contento_assert( current_context() == post.author, "cannot call from any other context" );
...
void apply_social_vote() {
   ...
   if( context == vote.author ) {
      ...
      contento_assert( Db::get( vote.postid, post ) > 0, "unable to find post" );
      contento_assert( now() - post.created < days(7), "cannot vote after 7 days" );
      ...
      contento_assert( abs_vote <= vote_account.vote_power, "insufficient vote power" );
      post.votes += vote.vote_power;
      vote_account.vote_power -= abs_vote;
      vote_account.last_vote  = now();
      Db::store( "account", vote_account );
   } else {
      contento_assert( false, "invalid context for execution of this vote" );
   }
}
