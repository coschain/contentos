...
void apply_context::exec()
{
   ...
   for( const auto& inline_op : _inline_ops ) {
      trx_context.apply( inline_op, namex(inline_op.contract_name), false, recurse_depth + 1 );
   }
...
void apply_context::execute_inline( vm_operation&& op ) {
   auto* code = control.db().find<account_object, by_name>(namex(op.contract_name));
...
const table_id_object* apply_context::find_table( namex code, name scope, name table ) {
   return db.find<table_id_object, by_code_scope_table>(boost::make_tuple(code, scope, table));
}

const table_id_object& apply_context::find_or_create_table( namex code, name scope, name table, const account_name &payer ) {
...
int apply_context::db_store_i64( namex code, uint64_t scope, uint64_t table, const account_name& payer, uint64_t id, const char* buffer, size_t buffer_size ) {
...
int apply_context::db_find_i64( namex code, uint64_t scope, uint64_t table, uint64_t id ) {
...
int apply_context::db_lowerbound_i64( namex code, uint64_t scope, uint64_t table, uint64_t id ) {
   //require_read_lock( code, scope ); // redundant?
...
int apply_context::db_upperbound_i64( namex code, uint64_t scope, uint64_t table, uint64_t id ) {
   //require_read_lock( code, scope ); // redundant?
...
int apply_context::db_end_i64( namex code, uint64_t scope, uint64_t table ) {
   //require_read_lock( code, scope ); // redundant?
