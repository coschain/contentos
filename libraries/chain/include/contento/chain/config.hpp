...
namespace contento { namespace chain { namespace config {
...
} } } // namespace contento::chain::config

constexpr uint64_t EOS_PERCENT(uint64_t value, uint32_t percentage) {
   return (value * percentage) / contento::chain::config::percent_100;
}

template<typename Number>
Number EOS_PERCENT_CEIL(Number value, uint32_t percentage) {
   return ((value * percentage) + contento::chain::config::percent_100 - contento::chain::config::percent_1)  / contento::chain::config::percent_100;
}
