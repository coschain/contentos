...
//               context.require_write_lock( table_obj.scope );
            ...
            int find_secondary( namex code, uint64_t scope, uint64_t table, secondary_key_proxy_const_type secondary, uint64_t& primary ) {
            ...
            int lowerbound_secondary( namex code, uint64_t scope, uint64_t table, secondary_key_proxy_type secondary, uint64_t& primary ) {
               auto tab = context.find_table( code, scope, table );
               if( !tab ) return -1;
            ...
            int upperbound_secondary( namex code, uint64_t scope, uint64_t table, secondary_key_proxy_type secondary, uint64_t& primary ) {
               auto tab = context.find_table( code, scope, table );
               if( !tab ) return -1;
            ...
            int end_secondary( namex code, uint64_t scope, uint64_t table ) {
               auto tab = context.find_table( code, scope, table );
               if( !tab ) return -1;
            ...
            int find_primary( namex code, uint64_t scope, uint64_t table, secondary_key_proxy_type secondary, uint64_t primary ) {
               auto tab = context.find_table( code, scope, table );
               if( !tab ) return -1;
            ...
            int lowerbound_primary( namex code, uint64_t scope, uint64_t table, uint64_t primary ) {
               auto tab = context.find_table( code, scope, table );
               if (!tab) return -1;
            ...
            int upperbound_primary( namex code, uint64_t scope, uint64_t table, uint64_t primary ) {
               auto tab = context.find_table( code, scope, table );
               if ( !tab ) return -1;
   ...
   private:
      ...
      const table_id_object* find_table( namex code, name scope, name table );
      const table_id_object& find_or_create_table( namex code, name scope, name table, const account_name &payer );
      void                   remove_table( const table_id_object& tid );

      int  db_store_i64( namex code, uint64_t scope, uint64_t table, const account_name& payer, uint64_t id, const char* buffer, size_t buffer_size );
   ...
   public:
      ...
      bool excute_operation( const std::vector<char>& op_buff );
...
//FC_REFLECT(contento::chain::apply_context::apply_results, (applied_actions)(deferred_transaction_requests)(deferred_transactions_count))
