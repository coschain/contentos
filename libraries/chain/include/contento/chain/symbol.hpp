...
#include <contento/chain/core_symbol.hpp>
#include <string>
#include <functional>
...
#define SY(P,X) ::contento::chain::string_to_symbol_c(P,#X)
      ...
      class symbol {
         public:
            ...
            explicit operator string() const
            {
               uint64_t v = m_value;
               uint8_t p = v & 0xFF;
               string ret = contento::chain::to_string(p);
               ret += ',';
               ret += name();
               return ret;
            }
...
namespace fc {
   inline void to_variant(const contento::chain::symbol& var, fc::variant& vo) { vo = var.to_string(); }
   inline void from_variant(const fc::variant& var, contento::chain::symbol& vo) {
      vo = contento::chain::symbol::from_string(var.get_string());
   }
}

namespace fc {
   inline void to_variant(const contento::chain::symbol_code& var, fc::variant& vo) {
      vo = contento::chain::symbol(var.value << 8).name();
   }
   inline void from_variant(const fc::variant& var, contento::chain::symbol_code& vo) {
      vo = contento::chain::symbol(0, var.get_string().c_str()).to_symbol_code();
   }
}

FC_REFLECT(contento::chain::symbol_code, (value))
FC_REFLECT(contento::chain::symbol, (m_value))
FC_REFLECT(contento::chain::extended_symbol, (sym)(contract))
