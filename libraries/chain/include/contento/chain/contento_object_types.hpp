...
using contento::protocol::transaction_id_type;
using contento::protocol::chain_id_type;
...
{
   class variant;
   inline void to_variant( const contento::chain::shared_string& s, variant& var )
   {
      var = fc::string( contento::chain::to_string( s ) );
   }

   inline void from_variant( const variant& var, contento::chain::shared_string& s )
   {
   ...
   namespace raw
   {
      ...
      template< typename T > inline void pack( contento::chain::buffer_type& raw, const T& v )
      {
      ...
      template< typename T > inline void unpack( const contento::chain::buffer_type& raw, T& v )
      {
         datastream< const char* > ds( raw.data(), raw.size() );
         unpack( ds, v );
      }

      template< typename T > inline T unpack( const contento::chain::buffer_type& raw )
      {
         T v;
         datastream< const char* > ds( raw.data(), raw.size() );
         unpack( ds, v );
...
FC_REFLECT_TYPENAME( contento::chain::buffer_type )

FC_REFLECT_ENUM( contento::chain::bandwidth_type, (post)(forum)(market) )
