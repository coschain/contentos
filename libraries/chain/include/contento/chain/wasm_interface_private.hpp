...
#include <contento/chain/webassembly/binaryen.hpp>
...
using namespace fc;
using namespace contento::chain::webassembly;
using namespace IR;
...
#define _REGISTER_INJECTED_INTRINSIC_WITH_PRICE(R, CLS, INFO_PRICE)\
    _REGISTER_INTRINSIC_WITH_PRICE_IN_MOD(CLS, contento_INJECTED_MODULE_NAME, INFO_PRICE)
...
// for basic intrinsic prices
#define SET_BASIC_INTRINSIC_PRICE(ID, PRICE)  _SET_BINARYEN_BASIC_INTRINSIC_PRICE(ID, PRICE)
#define _SET_BASIC_INTRINSIC_PRICE(R, PREFIX, ID)  SET_BASIC_INTRINSIC_PRICE( ::wasm::Expression::ID, BOOST_PP_CAT(PREFIX, ID) );
#define SET_BASIC_INTRINSIC_PRICES(PRICE_MACRO_PREFIX, ID_SEQ)    BOOST_PP_SEQ_FOR_EACH(_SET_BASIC_INTRINSIC_PRICE, PRICE_MACRO_PREFIX, ID_SEQ)
