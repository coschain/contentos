...
void set_withdraw_vesting_route_evaluator::do_apply( const set_withdraw_vesting_route_operation& o )
{
   ...
   if( itr == wd_idx.end() )
   {
      ...
      FC_ASSERT( from_account.withdraw_routes < CONTENTO_MAX_WITHDRAW_ROUTES, "Account already has the maximum number of routes." );
   ...
   FC_ASSERT( total_percent <= CONTENTO_100_PERCENT, "More than 100% of vesting withdrawals allocated to destinations." );
   }
...
//            cvo.vote_percent = o.weight;
   ...
   // used_power = (current_power * abs_weight / CONTENTO_100_PERCENT) * (reserve / max_vote_denom)
...
//    if( db.has_hardfork( CONTENTO_HARDFORK_0_13__256 ) )
   ...
   if( db.head_block_num() < CONTENTO_START_MINER_VOTING_BLOCK )
      pow_reward.amount *= CONTENTO_MAX_WITNESSES;
...
void transfer_from_savings_evaluator::do_apply( const transfer_from_savings_operation& op )
{
   ...
   FC_ASSERT( from.savings_withdraw_requests < CONTENTO_SAVINGS_WITHDRAW_REQUEST_LIMIT, "Account has reached limit for pending withdraw requests." );
   ...
   _db.create<savings_withdraw_object>( [&]( savings_withdraw_object& s ) {
      ...
      s.complete = _db.head_block_time() + CONTENTO_SAVINGS_WITHDRAW_TIME;
   });
...
//   FC_ASSERT( _db.has_hardfork( CONTENTO_HARDFORK_0_14__324 ) );
   ...
   if( o.decline )
   {
      ...
      _db.create< decline_voting_rights_request_object >( [&]( decline_voting_rights_request_object& req )
      {
         ...
         req.effective_date = _db.head_block_time() + CONTENTO_OWNER_AUTH_RECOVERY_PERIOD;
      });
   }
   else
   {
...
void delegate_vesting_shares_evaluator::do_apply( const delegate_vesting_shares_operation& op )
{
   ...
   else /* delegation->vesting_shares > op.vesting_shares */
   {
      ...
      _db.create< vesting_delegation_expiration_object >( [&]( vesting_delegation_expiration_object& obj )
      {
         ...
         obj.expiration = std::max( _db.head_block_time() + CONTENTO_CASHOUT_WINDOW_SECONDS, delegation->min_delegation_time );
      });
