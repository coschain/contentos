...
class softfloat_api : public context_aware_api {
   public:
      ...
      float _contento_f32_max( float af, float bf ) {
         float32_t a = to_softfloat32(af);
         float32_t b = to_softfloat32(bf);
         if (is_nan(a)) {
            return af;
      ...
      float _contento_f32_copysign( float af, float bf ) {
         float32_t a = to_softfloat32(af);
         float32_t b = to_softfloat32(bf);
      ...
      // float unops
      float _contento_f32_abs( float af ) {
         float32_t a = to_softfloat32(af);
         a.v &= ~(1 << 31);
         return from_softfloat32(a);
      }
      float _contento_f32_neg( float af ) {
         float32_t a = to_softfloat32(af);
      ...
      float _contento_f32_floor( float af ) {
         float32_t a = to_softfloat32(af);
         int e = (int)(a.v >> 23 & 0xFF) - 0X7F;
         uint32_t m;
         if (e >= 23)
      ...
      float _contento_f32_trunc( float af ) {
         float32_t a = to_softfloat32(af);
      ...
      float _contento_f32_nearest( float af ) {
         float32_t a = to_softfloat32(af);
         int e = a.v>>23 & 0xff;
         int s = a.v>>31;
         float32_t y;
      ...
      // float relops
      bool _contento_f32_eq( float a, float b ) {  return f32_eq( to_softfloat32(a), to_softfloat32(b) ); }
      bool _contento_f32_ne( float a, float b ) { return !f32_eq( to_softfloat32(a), to_softfloat32(b) ); }
      bool _contento_f32_lt( float a, float b ) { return f32_lt( to_softfloat32(a), to_softfloat32(b) ); }
      bool _contento_f32_le( float a, float b ) { return f32_le( to_softfloat32(a), to_softfloat32(b) ); }
      bool _contento_f32_gt( float af, float bf ) {
         float32_t a = to_softfloat32(af);
         float32_t b = to_softfloat32(bf);
         if (is_nan(a))
            return false;
      ...
      bool _contento_f32_ge( float af, float bf ) {
         float32_t a = to_softfloat32(af);
         float32_t b = to_softfloat32(bf);
         if (is_nan(a))
            return false;
      ...
      double _contento_f64_max( double af, double bf ) {
         float64_t a = to_softfloat64(af);
         float64_t b = to_softfloat64(bf);
         if (is_nan(a))
            return af;
      ...
      double _contento_f64_copysign( double af, double bf ) {
         float64_t a = to_softfloat64(af);
         float64_t b = to_softfloat64(bf);
      ...
      // double unops
      double _contento_f64_abs( double af ) {
         float64_t a = to_softfloat64(af);
         a.v &= ~(uint64_t(1) << 63);
         return from_softfloat64(a);
      }
      double _contento_f64_neg( double af ) {
         float64_t a = to_softfloat64(af);
      ...
      double _contento_f64_sqrt( double a ) {
         float64_t ret = f64_sqrt( to_softfloat64(a) );
         return from_softfloat64(ret);
      }
      // ceil, floor, trunc and nearest are lifted from libc
      double _contento_f64_ceil( double af ) {
         float64_t a = to_softfloat64( af );
         float64_t ret;
         int e = a.v >> 52 & 0x7ff;
         float64_t y;
      ...
      double _contento_f64_floor( double af ) {
         float64_t a = to_softfloat64( af );
         float64_t ret;
         int e = a.v >> 52 & 0x7FF;
         float64_t y;
      ...
      double _contento_f64_trunc( double af ) {
         float64_t a = to_softfloat64( af );
      ...
      double _contento_f64_nearest( double af ) {
         float64_t a = to_softfloat64( af );
         int e = (a.v >> 52 & 0x7FF);
         int s = a.v >> 63;
         float64_t y;
      ...
      // double relops
      bool _contento_f64_eq( double a, double b ) { return f64_eq( to_softfloat64(a), to_softfloat64(b) ); }
      bool _contento_f64_ne( double a, double b ) { return !f64_eq( to_softfloat64(a), to_softfloat64(b) ); }
      bool _contento_f64_lt( double a, double b ) { return f64_lt( to_softfloat64(a), to_softfloat64(b) ); }
      bool _contento_f64_le( double a, double b ) { return f64_le( to_softfloat64(a), to_softfloat64(b) ); }
      bool _contento_f64_gt( double af, double bf ) {
         float64_t a = to_softfloat64(af);
         float64_t b = to_softfloat64(bf);
         if (is_nan(a))
            return false;
      ...
      bool _contento_f64_ge( double af, double bf ) {
         float64_t a = to_softfloat64(af);
         float64_t b = to_softfloat64(bf);
         if (is_nan(a))
            return false;
      ...
      double _contento_f32_promote( float a ) {
         return from_softfloat64(f32_to_f64( to_softfloat32(a)) );
      }
      float _contento_f64_demote( double a ) {
         return from_softfloat32(f64_to_f32( to_softfloat64(a)) );
      }
      int32_t _contento_f32_trunc_i32s( float af ) {
         float32_t a = to_softfloat32(af);
         if (_contento_f32_ge(af, 2147483648.0f) || _contento_f32_lt(af, -2147483648.0f))
            FC_THROW_EXCEPTION( contento::chain::wasm_execution_error, "Error, f32.convert_s/i32 overflow" );

         if (is_nan(a))
            FC_THROW_EXCEPTION( contento::chain::wasm_execution_error, "Error, f32.convert_s/i32 unrepresentable");
         return f32_to_i32( to_softfloat32(_contento_f32_trunc( af )), 0, false );
      }
      int32_t _contento_f64_trunc_i32s( double af ) {
         float64_t a = to_softfloat64(af);
         if (_contento_f64_ge(af, 2147483648.0) || _contento_f64_lt(af, -2147483648.0))
            FC_THROW_EXCEPTION( contento::chain::wasm_execution_error, "Error, f64.convert_s/i32 overflow");
         if (is_nan(a))
            FC_THROW_EXCEPTION( contento::chain::wasm_execution_error, "Error, f64.convert_s/i32 unrepresentable");
         return f64_to_i32( to_softfloat64(_contento_f64_trunc( af )), 0, false );
      }
      uint32_t _contento_f32_trunc_i32u( float af ) {
         float32_t a = to_softfloat32(af);
         if (_contento_f32_ge(af, 4294967296.0f) || _contento_f32_le(af, -1.0f))
            FC_THROW_EXCEPTION( contento::chain::wasm_execution_error, "Error, f32.convert_u/i32 overflow");
         if (is_nan(a))
            FC_THROW_EXCEPTION( contento::chain::wasm_execution_error, "Error, f32.convert_u/i32 unrepresentable");
         return f32_to_ui32( to_softfloat32(_contento_f32_trunc( af )), 0, false );
      }
      uint32_t _contento_f64_trunc_i32u( double af ) {
         float64_t a = to_softfloat64(af);
         if (_contento_f64_ge(af, 4294967296.0) || _contento_f64_le(af, -1.0))
            FC_THROW_EXCEPTION( contento::chain::wasm_execution_error, "Error, f64.convert_u/i32 overflow");
         if (is_nan(a))
            FC_THROW_EXCEPTION( contento::chain::wasm_execution_error, "Error, f64.convert_u/i32 unrepresentable");
         return f64_to_ui32( to_softfloat64(_contento_f64_trunc( af )), 0, false );
      }
      int64_t _contento_f32_trunc_i64s( float af ) {
         float32_t a = to_softfloat32(af);
         if (_contento_f32_ge(af, 9223372036854775808.0f) || _contento_f32_lt(af, -9223372036854775808.0f))
            FC_THROW_EXCEPTION( contento::chain::wasm_execution_error, "Error, f32.convert_s/i64 overflow");
         if (is_nan(a))
            FC_THROW_EXCEPTION( contento::chain::wasm_execution_error, "Error, f32.convert_s/i64 unrepresentable");
         return f32_to_i64( to_softfloat32(_contento_f32_trunc( af )), 0, false );
      }
      int64_t _contento_f64_trunc_i64s( double af ) {
         float64_t a = to_softfloat64(af);
         if (_contento_f64_ge(af, 9223372036854775808.0) || _contento_f64_lt(af, -9223372036854775808.0))
            FC_THROW_EXCEPTION( contento::chain::wasm_execution_error, "Error, f64.convert_s/i64 overflow");
         if (is_nan(a))
            FC_THROW_EXCEPTION( contento::chain::wasm_execution_error, "Error, f64.convert_s/i64 unrepresentable");

         return f64_to_i64( to_softfloat64(_contento_f64_trunc( af )), 0, false );
      }
      uint64_t _contento_f32_trunc_i64u( float af ) {
         float32_t a = to_softfloat32(af);
         if (_contento_f32_ge(af, 18446744073709551616.0f) || _contento_f32_le(af, -1.0f))
            FC_THROW_EXCEPTION( contento::chain::wasm_execution_error, "Error, f32.convert_u/i64 overflow");
         if (is_nan(a))
            FC_THROW_EXCEPTION( contento::chain::wasm_execution_error, "Error, f32.convert_u/i64 unrepresentable");
         return f32_to_ui64( to_softfloat32(_contento_f32_trunc( af )), 0, false );
      }
      uint64_t _contento_f64_trunc_i64u( double af ) {
         float64_t a = to_softfloat64(af);
         if (_contento_f64_ge(af, 18446744073709551616.0) || _contento_f64_le(af, -1.0))
            FC_THROW_EXCEPTION( contento::chain::wasm_execution_error, "Error, f64.convert_u/i64 overflow");
         if (is_nan(a))
            FC_THROW_EXCEPTION( contento::chain::wasm_execution_error, "Error, f64.convert_u/i64 unrepresentable");
         return f64_to_ui64( to_softfloat64(_contento_f64_trunc( af )), 0, false );
      }
      float _contento_i32_to_f32( int32_t a )  {
         return from_softfloat32(i32_to_f32( a ));
      }
      float _contento_i64_to_f32( int64_t a ) {
         return from_softfloat32(i64_to_f32( a ));
      }
      float _contento_ui32_to_f32( uint32_t a ) {
         return from_softfloat32(ui32_to_f32( a ));
      }
      float _contento_ui64_to_f32( uint64_t a ) {
         return from_softfloat32(ui64_to_f32( a ));
      }
      double _contento_i32_to_f64( int32_t a ) {
         return from_softfloat64(i32_to_f64( a ));
      }
      double _contento_i64_to_f64( int64_t a ) {
         return from_softfloat64(i64_to_f64( a ));
      }
      double _contento_ui32_to_f64( uint32_t a ) {
         return from_softfloat64(ui32_to_f64( a ));
      }
      double _contento_ui64_to_f64( uint64_t a ) {
...
class crypto_api : public context_aware_api {
   public:
      ...
      template<class Encoder> auto encode(char* data, size_t datalen) {
         Encoder e;
         const size_t bs = contento::chain::config::hashing_checktime_block_size;
         while ( datalen > bs ) {
            e.write( data, bs );
            data += bs;
            datalen -= bs;
...
public:
   ...
   // Kept as intrinsic rather than implementing on WASM side (using contento_assert_message and strlen) because strlen is faster on native side.
   void contento_assert( bool condition, null_terminated_ptr msg ) {
      if( BOOST_UNLIKELY( !condition ) ) {
         std::string message( msg );
         edump((message));
         EOS_THROW( contento_assert_message_exception, "assertion failure with message: ${s}", ("s",message) );
      }
   }

   void contento_assert_message( bool condition, array_ptr<const char> msg, size_t msg_len ) {
      if( BOOST_UNLIKELY( !condition ) ) {
   ...
   void contento_assert_code( bool condition, uint64_t error_code ) {
      if( BOOST_UNLIKELY( !condition ) ) {
         edump((error_code));
         EOS_THROW( contento_assert_code_exception,
                    "assertion failure with error code: ${error_code}", ("error_code", error_code) );
      }
   }

   void contento_exit(int32_t code) {
...
#define DB_API_METHOD_WRAPPERS_SIMPLE_SECONDARY(IDX, TYPE)\
      int db_##IDX##_store( uint64_t scope, uint64_t table, namex& payer, uint64_t id, const TYPE& secondary ) {\
         return context.IDX.store( scope, table, payer, id, secondary );\
      }\
      void db_##IDX##_update( int iterator, namex& payer, const TYPE& secondary ) {\
         return context.IDX.update( iterator, payer, secondary );\
      }\
      void db_##IDX##_remove( int iterator ) {\
         return context.IDX.remove( iterator );\
      }\
      int db_##IDX##_find_secondary( namex& code, uint64_t scope, uint64_t table, const TYPE& secondary, uint64_t& primary ) {\
         return context.IDX.find_secondary(code, scope, table, secondary, primary);\
      }\
      int db_##IDX##_find_primary( namex& code, uint64_t scope, uint64_t table, TYPE& secondary, uint64_t primary ) {\
         return context.IDX.find_primary(code, scope, table, secondary, primary);\
      }\
      int db_##IDX##_lowerbound( namex& code, uint64_t scope, uint64_t table,  TYPE& secondary, uint64_t& primary ) {\
         return context.IDX.lowerbound_secondary(code, scope, table, secondary, primary);\
      }\
      int db_##IDX##_upperbound( namex& code, uint64_t scope, uint64_t table,  TYPE& secondary, uint64_t& primary ) {\
         return context.IDX.upperbound_secondary(code, scope, table, secondary, primary);\
      }\
      int db_##IDX##_end( namex& code, uint64_t scope, uint64_t table ) {\
...
#define DB_API_METHOD_WRAPPERS_ARRAY_SECONDARY(IDX, ARR_SIZE, ARR_ELEMENT_TYPE)\
      int db_##IDX##_store( uint64_t scope, uint64_t table, namex& payer, uint64_t id, array_ptr<const ARR_ELEMENT_TYPE> data, size_t data_len) {\
      ...
      void db_##IDX##_update( int iterator, namex& payer, array_ptr<const ARR_ELEMENT_TYPE> data, size_t data_len ) {\
         FC_ASSERT( data_len == ARR_SIZE,\
                    "invalid size of secondary key array for " #IDX ": given ${given} bytes but expected ${expected} bytes",\
                    ("given",data_len)("expected",ARR_SIZE) );\
      ...
      int db_##IDX##_find_secondary( namex& code, uint64_t scope, uint64_t table, array_ptr<const ARR_ELEMENT_TYPE> data, size_t data_len, uint64_t& primary ) {\
         FC_ASSERT( data_len == ARR_SIZE,\
                    "invalid size of secondary key array for " #IDX ": given ${given} bytes but expected ${expected} bytes",\
                    ("given",data_len)("expected",ARR_SIZE) );\
         return context.IDX.find_secondary(code, scope, table, data, primary);\
      }\
      int db_##IDX##_find_primary( namex& code, uint64_t scope, uint64_t table, array_ptr<ARR_ELEMENT_TYPE> data, size_t data_len, uint64_t primary ) {\
         FC_ASSERT( data_len == ARR_SIZE,\
                    "invalid size of secondary key array for " #IDX ": given ${given} bytes but expected ${expected} bytes",\
                    ("given",data_len)("expected",ARR_SIZE) );\
         return context.IDX.find_primary(code, scope, table, data.value, primary);\
      }\
      int db_##IDX##_lowerbound( namex& code, uint64_t scope, uint64_t table, array_ptr<ARR_ELEMENT_TYPE> data, size_t data_len, uint64_t& primary ) {\
         FC_ASSERT( data_len == ARR_SIZE,\
                    "invalid size of secondary key array for " #IDX ": given ${given} bytes but expected ${expected} bytes",\
                    ("given",data_len)("expected",ARR_SIZE) );\
         return context.IDX.lowerbound_secondary(code, scope, table, data.value, primary);\
      }\
      int db_##IDX##_upperbound( namex& code, uint64_t scope, uint64_t table, array_ptr<ARR_ELEMENT_TYPE> data, size_t data_len, uint64_t& primary ) {\
         FC_ASSERT( data_len == ARR_SIZE,\
                    "invalid size of secondary key array for " #IDX ": given ${given} bytes but expected ${expected} bytes",\
                    ("given",data_len)("expected",ARR_SIZE) );\
...
class database_api : public context_aware_api {
   public:
      ...
      int db_store_i64( uint64_t scope, uint64_t table, namex& payer, uint64_t id, array_ptr<const char> buffer, size_t buffer_size ) {
         return context.db_store_i64( scope, table, payer, id, buffer, buffer_size );
      }
      void db_update_i64( int itr, namex& payer, array_ptr<const char> buffer, size_t buffer_size ) {
      ...
      int db_find_i64( namex& code, uint64_t scope, uint64_t table, uint64_t id ) {
         return context.db_find_i64( code, scope, table, id );
      }
      int db_lowerbound_i64( namex& code, uint64_t scope, uint64_t table, uint64_t id ) {
         return context.db_lowerbound_i64( code, scope, table, id );
      }
      int db_upperbound_i64( namex& code, uint64_t scope, uint64_t table, uint64_t id ) {
         return context.db_upperbound_i64( code, scope, table, id );
      }
      int db_end_i64( namex& code, uint64_t scope, uint64_t table ) {
...
//   WITH_PRICE (set_resource_limits,              void(int,int64_t,int64_t,int64_t) )
//   WITH_PRICE (set_proposed_producers,           int64_t(int,int)                      )
   ...
   WITH_PRICE (is_privileged,                    int(int)                          )
   WITH_PRICE (set_privileged,                   void(int, int)                    )
);
...
#define DB_SECONDARY_INDEX_METHODS_SIMPLE(IDX) \
   WITH_PRICE (db_##IDX##_store,          int(int64_t,int64_t,int,int64_t,int))\
   WITH_PRICE (db_##IDX##_remove,         void(int))\
   WITH_PRICE (db_##IDX##_update,         void(int,int,int))\
   WITH_PRICE (db_##IDX##_find_primary,   int(int,int64_t,int64_t,int,int64_t))\
   WITH_PRICE (db_##IDX##_find_secondary, int(int,int64_t,int64_t,int,int))\
   WITH_PRICE (db_##IDX##_lowerbound,     int(int,int64_t,int64_t,int,int))\
   WITH_PRICE (db_##IDX##_upperbound,     int(int,int64_t,int64_t,int,int))\
   WITH_PRICE (db_##IDX##_end,            int(int,int64_t,int64_t))\
   WITH_PRICE (db_##IDX##_next,           int(int, int))\
   WITH_PRICE (db_##IDX##_previous,       int(int, int))

#define DB_SECONDARY_INDEX_METHODS_ARRAY(IDX) \
      WITH_PRICE (db_##IDX##_store,          int(int64_t,int64_t,int,int64_t,int, int))\
      WITH_PRICE (db_##IDX##_remove,         void(int))\
      WITH_PRICE (db_##IDX##_update,         void(int,int,int,int))\
      WITH_PRICE (db_##IDX##_find_primary,   int(int,int64_t,int64_t,int,int,int64_t))\
      WITH_PRICE (db_##IDX##_find_secondary, int(int,int64_t,int64_t,int,int,int))\
      WITH_PRICE (db_##IDX##_lowerbound,     int(int,int64_t,int64_t,int,int,int))\
      WITH_PRICE (db_##IDX##_upperbound,     int(int,int64_t,int64_t,int,int,int))\
      WITH_PRICE (db_##IDX##_end,            int(int,int64_t,int64_t))\
      WITH_PRICE (db_##IDX##_next,           int(int, int))\
      WITH_PRICE (db_##IDX##_previous,       int(int, int))

REGISTER_INTRINSICS_WITH_PRICE( database_api,
   WITH_PRICE (db_store_i64,        int(int64_t,int64_t,int,int64_t,int,int))
   WITH_PRICE (db_update_i64,       void(int,int,int,int))
   ...
   WITH_PRICE (db_find_i64,         int(int,int64_t,int64_t,int64_t))
   WITH_PRICE (db_lowerbound_i64,   int(int,int64_t,int64_t,int64_t))
   WITH_PRICE (db_upperbound_i64,   int(int,int64_t,int64_t,int64_t))
   WITH_PRICE (db_end_i64,          int(int,int64_t,int64_t))
...
REGISTER_INTRINSICS_WITH_PRICE(transaction_api,
   ...
   WITH_PRICE (send_deferred,             void(int, int, int, int, int32_t) )
...
std::istream& operator>>(std::istream& in, wasm_interface::vm_type& runtime) {
   ...
   in >> s;
   if (s == "wavm")
      runtime = contento::chain::wasm_interface::vm_type::wavm;
   else if (s == "binaryen")
      runtime = contento::chain::wasm_interface::vm_type::binaryen;
   else
