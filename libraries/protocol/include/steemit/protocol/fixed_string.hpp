...
namespace fc { namespace raw {
   ...
   inline void pack( Stream& s, const contento::protocol::fixed_string< Storage >& u )
   {
      pack( s, std::string( u ) );
   }

   template< typename Stream, typename Storage >
   inline void unpack( Stream& s, contento::protocol::fixed_string< Storage >& u )
   {
      std::string str;
      unpack( s, str );
      u = str;
   ...
   void to_variant(   const contento::protocol::fixed_string< Storage >& s, variant& v ) { v = std::string( s ); }

   template< typename Storage >
   void from_variant( const variant& v, contento::protocol::fixed_string< Storage >& s ) { s = v.as_string(); }
} // fc
