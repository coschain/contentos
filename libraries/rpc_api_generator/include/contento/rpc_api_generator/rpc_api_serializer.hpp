 ...
 */
struct rpc_api_serializer {
   rpc_api_serializer(){ configure_built_in_types(); }
   rpc_api_serializer( const abi_def& abi );
...
namespace impl {
   ...
   struct abi_to_variant {
      ...
      {
         FC_ASSERT( ++recursion_depth < rpc_api_serializer::max_recursion_depth, "recursive definition, max_recursion_depth" );
         mvo(name,v);
      }
...
template<typename T, typename Resolver>
void rpc_api_serializer::to_variant( const T& o, variant& vo, Resolver resolver ) try {
   mutable_variant_object mvo;
...
template<typename T, typename Resolver>
void rpc_api_serializer::from_variant( const variant& v, T& o, Resolver resolver ) try {
