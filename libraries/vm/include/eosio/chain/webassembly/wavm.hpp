...
namespace contento { namespace chain { namespace webassembly { namespace wavm {

using namespace IR;
using namespace Runtime;
using namespace fc;
using namespace contento::chain::webassembly::common;

class wavm_runtime : public contento::chain::wasm_runtime_interface {
   public:
      wavm_runtime();
      ~wavm_runtime();
...
#define _REGISTER_WAVM_INTRINSIC(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)\
   static Intrinsics::Function _INTRINSIC_NAME(__intrinsic_fn, __COUNTER__) (\
      MOD "." NAME,\
      contento::chain::webassembly::wavm::wasm_function_type_provider<WASM_SIG>::type(),\
      (void *)contento::chain::webassembly::wavm::intrinsic_function_invoker_wrapper<WASM_SIG, SIG>::type::fn<&CLS::METHOD>()\
   );\
