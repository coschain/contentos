...
namespace contento { namespace chain { namespace wasm_validations {
   ...
   class wasm_binary_validation {
      ...
      public:
         wasm_binary_validation( const contento::chain::controller& control, IR::Module& mod ) : _module( &mod ) {
         ...
         void validate() {
            ...
            for ( auto& fd : _module->functions.defs ) {
               wasm_ops::contento_OperatorDecoderStream<op_constrainers> decoder(fd.code);
               while ( decoder ) {
