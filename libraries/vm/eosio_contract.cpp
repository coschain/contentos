...
#include <contento/chain/permission_object.hpp>
#include <contento/chain/permission_link_object.hpp>
#include <contento/chain/global_property_object.hpp>
#include <contento/chain/contract_types.hpp>
#include <contento/chain/producer_object.hpp>
...
void validate_authority_precondition( const apply_context& context, const authority& auth ) {
   for(const auto& a : auth.accounts) {
      ...
      if( a.permission.permission == config::contento_code_name ) // virtual contento.code permission does not really exist but is allowed
         continue;
 ...
 */
void apply_contento_newaccount(apply_context& context) {
   auto create = context.act.data_as<newaccount>();
   try {
   context.require_authorization(create.creator);
//   context.require_write_lock( config::contento_auth_scope );
   ...
   if( !creator.privileged ) {
      EOS_ASSERT( name_str.find( "contento." ) != 0, action_validate_exception,
                  "only privileged accounts can have names that start with 'contento.'" );
   }
...
void apply_contento_updateauth(apply_context& context) {
   ...
   EOS_ASSERT( update.permission.to_string().find( "contento." ) != 0, action_validate_exception,
               "Permission names that start with 'contento.' are reserved" );
...
void apply_contento_deleteauth(apply_context& context) {
//   context.require_write_lock( config::contento_auth_scope );
...
void apply_contento_linkauth(apply_context& context) {
//   context.require_write_lock( config::contento_auth_scope );
   ...
   try {
      ...
      if( requirement.requirement != config::contento_any_name ) {
...
void apply_contento_unlinkauth(apply_context& context) {
//   context.require_write_lock( config::contento_auth_scope );
...
void apply_contento_canceldelay(apply_context& context) {
  //// Y
