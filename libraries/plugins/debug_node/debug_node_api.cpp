...
class debug_private_key_storage : public private_key_storage
{
   public:
      ...
      virtual void maybe_get_private_key(
         ...
         const contento::chain::public_key_type& pubkey,
         const std::string& account_name
         ) override;

      std::string                dev_key_prefix;
      std::map< contento::chain::public_key_type, fc::ecc::private_key > key_table;
};

class debug_node_api_impl
{
   public:
      debug_node_api_impl( contento::app::application& _app );
      ...
      fc::optional< contento::chain::signed_block > debug_pop_block();
      //void debug_push_block( const contento::chain::signed_block& block );
      contento::chain::witness_schedule_object debug_get_witness_schedule();
      contento::chain::hardfork_property_object debug_get_hardfork_property_object();
      ...
      std::shared_ptr< contento::plugin::debug_node::debug_node_plugin > get_plugin();
...
)
{
   ...
   if( (pubkey == contento::chain::public_key_type()) || (gen_pub == pubkey) )
   {
      result = gen_priv;
      return;
   }
...
debug_node_api_impl::debug_node_api_impl( contento::app::application& _app ) : app( _app )
{
#ifdef CONTENTO_INIT_PRIVATE_KEY
   fc::ecc::private_key init_key = CONTENTO_INIT_PRIVATE_KEY;
...
void debug_node_api_impl::debug_mine( debug_mine_result& result, const debug_mine_args& args )
{
   ...
   tx.sign( *priv, CONTENTO_CHAIN_ID );
...
uint32_t debug_node_api_impl::debug_push_blocks( const std::string& src_filename, uint32_t count, bool skip_validate_invariants )
{
   ...
   std::shared_ptr< contento::chain::database > db = app.chain_database();
     ...
     )
   {
      ...
      contento::chain::block_log log;
      log.open( src_path );
      uint32_t first_block = db->head_block_num()+1;
      uint32_t skip_flags = contento::chain::database::skip_nothing;
      if( skip_validate_invariants )
         skip_flags = skip_flags | contento::chain::database::skip_validate_invariants;
      for( uint32_t i=0; i<count; i++ )
      {
         //fc::optional< contento::chain::signed_block > block = log.read_block( log.get_block_pos( first_block + i ) );
         uint64_t block_pos = log.get_block_pos( first_block + i );
         if( block_pos == contento::chain::block_log::npos )
         {
...
{
   return get_plugin()->debug_generate_blocks( debug_key, count, contento::chain::database::skip_nothing, 0, &key_storage );
}
...
{
   return get_plugin()->debug_generate_blocks_until( debug_key, head_block_time, generate_sparsely, contento::chain::database::skip_nothing, &key_storage );
}

fc::optional< contento::chain::signed_block > debug_node_api_impl::debug_pop_block()
{
   std::shared_ptr< contento::chain::database > db = app.chain_database();
   return db->fetch_block_by_number( db->head_block_num() );
}

/*void debug_node_api_impl::debug_push_block( const contento::chain::signed_block& block )
{
   app.chain_database()->push_block( block );
}*/

contento::chain::witness_schedule_object debug_node_api_impl::debug_get_witness_schedule()
{
   return app.chain_database()->get( contento::chain::witness_schedule_id_type() );
}

contento::chain::hardfork_property_object debug_node_api_impl::debug_get_hardfork_property_object()
{
   return app.chain_database()->get( contento::chain::hardfork_property_id_type() );
}
...
std::shared_ptr< contento::plugin::debug_node::debug_node_plugin > debug_node_api_impl::get_plugin()
{
...
void debug_node_api_impl::debug_set_hardfork( uint32_t hardfork_id )
{
   ...
   if( hardfork_id > CONTENTO_NUM_HARDFORKS )
      return;
...
{
   return app.chain_database()->get( contento::chain::hardfork_property_id_type() ).last_hardfork >= hardfork_id;
}
...
debug_node_api::debug_node_api( const contento::app::api_context& ctx )
{
...
fc::optional< contento::chain::signed_block > debug_node_api::debug_pop_block()
{
   return my->debug_pop_block();
}

/*void debug_node_api::debug_push_block( contento::chain::signed_block& block )
{
   my->debug_push_block( block );
}*/

contento::chain::witness_schedule_object debug_node_api::debug_get_witness_schedule()
{
   return my->debug_get_witness_schedule();
}

contento::chain::hardfork_property_object debug_node_api::debug_get_hardfork_property_object()
{
