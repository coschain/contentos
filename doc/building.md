...
Builds contentosd to be a consensus-only low memory node. Data and fields not
...
For Ubuntu 16.04 users, after installing the right packages with `apt` Contentos
...
Contentos requires Boost 1.58 (as in Ubuntu 16.04) and works with versions up to 1.60 (including).
So building Contentos on Ubuntu 14.04 requires downloading and installing a more recent
version of Boost.
...
Ubuntu 14.04) when compiling in C++11 mode (which Contentos does).
...
Then the instructions are the same as for contentos:
...
contentos. Until then, this will allow you to install boost 1.60.0.
